...
-------------------------------------------------------------------------*/
  var headerSticky = function () {
    ...
    const scrolledSrc = originalSrc || "/sarjan-assets/sarjan-logo-full.png";
...
------------------------------------------------------------------------------------- */
  var totalPriceVariant = function () {
    ...
    $(".tf-product-info-list,.tf-cart-item").each(function () {
      ...
      var basePrice =
        parseMoney(productItem.find(".price-on-sale").data("base-price")) ||
        parseMoney(productItem.find(".price-on-sale").text());
      var quantityInput = productItem.find(".quantity-product");

      productItem.find(".color-btn, .size-btn").on("click", function () {
        var newPrice = parseMoney($(this).data("price")) || basePrice;
        quantityInput.val(1);
        productItem
          .find(".price-on-sale")
      ...
      function updateTotalPrice(price, scope) {
        var currentPrice =
          price ||
          parseMoney(scope.find(".price-on-sale").data("base-price")) ||
          parseMoney(scope.find(".price-on-sale").text());
...
------------------------------------------------------------------------------------- */
  ...
  var scrollBottomSticky = function () {
    $(window).on("scroll", function () {
      ...
      var atcButton = $(".tf-product-info-by-btn");

      if (!atcButton.length || !atcButton.offset()) {
        myElement.removeClass("show");
        return;
      }

      // .tf-product-info-by-btn.offset().top
      var height = atcButton.offset().top + 50;
      // console.log(height);
