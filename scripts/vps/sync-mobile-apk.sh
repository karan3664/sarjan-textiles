...
VERSION_CODE="$(python3 -c "import json; print(json.load(open('$TMP_MANIFEST'))['versionCode'])")"

curl -fsSL "$APK_SOURCE" -o "$DEST.tmp"
mv "$DEST.tmp" "$DEST"
chmod 644 "$DEST"
echo "$VERSION_CODE" > "$DEST.version"
chmod 644 "$DEST.version"
ls -lh "$DEST" "$DEST.version"
echo "APK synced to $DEST (versionCode $VERSION_CODE)"
