#!/usr/bin/env node
/**
 * Process cotton printed shirt zip photos on LIVE via Admin AI Studio API.
 *
 *   node scripts/push-cotton-printed-shirts-live.mjs <csv> <zip>
 */
...
async function approve(token, id, sku, color) {
  const res = await fetch(`${LIVE_URL}/api/admin/ai-studio/action`, {
    ...
    body: JSON.stringify({
      ...
      action: "approve",
      sku,
      note: `Cotton printed shirt live push — ${sku} ${color}`,
    }),
  });
  const data = await res.json().catch(() => ({}));
...
async function main() {
  ...
  if (!csvPath || !zipPath) {
    throw new Error(
      "Usage: node scripts/push-cotton-printed-shirts-live.mjs <csv> <zip>",
    );
  }
  ...
  const extractDir = mkdtempSync(path.join(os.tmpdir(), "cotton-shirts-live-"));
  execSync(
