/**
 * Import Ajrakh shirts: CSV rows + zip photos → AI Product Studio → CMS.
 *
 ...
 *
 * Run:
 *   npx tsx --env-file=.env.local scripts/import-ajrakh-shirts-ai-studio.ts \
 *     "/path/Product upload sheet - Ajrakh Shirts.csv" \
 *     "/path/Ajrakh Shirts.zip"
 */
...
async function approveBySkuColor(
  ...
  const approved = await updateStudioRecord({
    ...
    action: "approve",
    sku,
    note: `Ajrakh batch import — ${sku} ${color}`,
  });
...
async function main() {
  ...
  if (!csvPath || !zipPath) {
    throw new Error(
      "Usage: npx tsx scripts/import-ajrakh-shirts-ai-studio.ts <csv> <zip>",
    );
  }
  ...
  const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "ajrakh-shirts-"));
  execSync(
