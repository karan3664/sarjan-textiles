/**
 * Import Mashru Ajrakh Kurtas: CSV rows + zip photos → AI Product Studio → CMS.
 *
 * Zip folders use 1..N (supports `7 E (1)`, `10E (2)`, `11.jpeg`, etc.).
 * Each CSV row maps to one product group; images map to colors in CSV order.
 *
 * Run:
 *   npx tsx --env-file=.env.local scripts/import-mashru-kurta-ai-studio.ts \
 *     "/path/Product upload sheet - Mashru Ajrakh Kurta.csv" \
 *     "/path/Mashru Ajrakh Kurta Upload 2 final.zip"
 */
...
async function approveBySkuColor(
  ...
  const approved = await updateStudioRecord({
    ...
    action: "approve",
    sku,
    note: `Mashru kurta batch import — ${sku} ${color}`,
  });
...
function productFromRow(
  ...
  const imageList =
    images.filter(Boolean).length > 0
      ...
      : variantColors.map(() => PRODUCT_PLACEHOLDER_IMAGE);
  ...
  return {
    ...
    fabric: stringValue(row, "fabric") || "Mashru",
    price,
...
async function main() {
  ...
  if (!csvPath || !zipPath) {
    throw new Error(
      "Usage: npx tsx scripts/import-mashru-kurta-ai-studio.ts <csv> <zip>",
    );
  }
  ...
  const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "mashru-kurta-"));
  execSync(
