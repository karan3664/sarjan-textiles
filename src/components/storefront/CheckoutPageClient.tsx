...
export function CheckoutPageClient() {
  ...
  const submitOrder = async () => {
    const client = JSON.parse(localStorage.getItem("sarjan-client") ?? "null") as { id?: string; email?: string } | null;
    if (!client?.id) {
    ...
    const res = await fetch("/api/orders", {
      ...
      body: JSON.stringify({
        ...
        subtotal,
        dispatchAddress: (document.querySelector<HTMLInputElement>("[name='dispatchAddress']")?.value ?? "").trim(),
        note: (document.querySelector<HTMLTextAreaElement>("[name='note']")?.value ?? "").trim(),
    ...
    setMessage(res.ok ? `Order request saved: ${data.order.id}` : data.error ?? "Order failed");
    if (res.ok) {
      writeCart([]);
      window.location.assign(`/payment-confirmation?orderId=${encodeURIComponent(data.order.id)}`);
    }
  };
        ...
        <div className="container">
          {loading ? (
                ...
                <div className="flat-spacing tf-page-checkout">
                  {!client?.id ? (
                    <div className="wrap">
                      <div className="title-login">
                        ...
                        <a href="/login" className="text-button">Login here</a>
                      </div>
                      <form className="login-box">
                        ...
                        <button className="tf-btn" type="button"><span className="text">Login</span></button>
                      </form>
                    </div>
                  ) : null}
                  <div className="wrap">
                    ...
                    <form className="info-box" key={client?.id ?? "guest"}>
                      <div className="grid-2">
                        <input type="text" placeholder="Company Name*" name="companyName" defaultValue={client?.companyName ?? ""} />
                        <input type="text" placeholder="Contact Person*" name="contactPerson" defaultValue={client?.address?.contactName ?? ""} />
                      </div>
                      <div className="grid-2">
                        <input type="text" placeholder="Email Address*" defaultValue={client?.email ?? ""} />
                        <input type="text" placeholder="Phone Number*" defaultValue={client?.phone ?? ""} />
                      </div>
                      <div className="tf-select">
                        <select className="text-title" name="address[country]" defaultValue="India">
                          <option value="Choose Country/Region">Choose Country/Region</option>
                          <option value="India">India</option>
                        </select>
                      </div>
                      <div className="grid-2">
                        <input type="text" placeholder="Town/City*" defaultValue={client?.address?.city ?? client?.city ?? ""} />
                        <input type="text" placeholder="Street, address..." name="dispatchAddress" defaultValue={client?.address?.line1 ?? ""} />
                      </div>
                      <div className="grid-2">
                        <div className="tf-select">
                  ...
                  <div className="wrap">
                    ...
                    <form className="form-payment">
                      ...
                      <button className="tf-btn btn-reset" type="button" onClick={submitOrder}>Submit Order Request</button>
                    </form>
                  </div>
                </div>
              </div>
                ...
                <div className="flat-spacing flat-sidebar-checkout">
                  <div className="sidebar-checkout-content">
                    ...
                    <div className="list-product">
                      {lines.map((item) => (
                        <div className="item-product" key={`${item.slug}-${item.color}-${item.sizes.join("-")}`}>
                          <Link href={`/products/${item.product.slug}`} className="img-product"><img src={item.product.images[0]} alt={item.product.name} /></Link>
                          <div className="content-box">
                            <div className="info">
                              <Link href={`/products/${item.product.slug}`} className="name-product link text-title">{item.product.name}</Link>
                    ...
                    <div className="sec-discount">
                      ...
                      <p>Discount code is only used after admin review for eligible B2B orders.</p>
                    </div>
            ...
            <div className="text-center py-5">
              ...
              <p className="text-secondary mt_8">Add product sets before checkout.</p>
