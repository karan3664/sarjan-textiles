...
function readClient() {
  ...
  try {
    return JSON.parse(localStorage.getItem("sarjan-client") ?? "null") as Client | null;
  } catch {
    return null;
  }
}
...
function useClientAndOrders() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/clients/${encodeURIComponent(stored.id)}`).then((res) => res.json()).catch(() => null),
      fetch(`/api/orders?clientId=${encodeURIComponent(stored.id)}`).then((res) => res.json()).catch(() => ({ orders: [] })),
...
function AccountFrame({ active, title, children }: { active: string; title: string; children: React.ReactNode }) {
  const [client, setClient] = useState<Client | null>(null);
  ...
  return (
    <>
      ...
      <div className="btn-sidebar-account"><button data-bs-toggle="offcanvas" data-bs-target="#mbAccount" aria-controls="mbAccount"><i className="icon icon-sidebar" /></button></div>
            ...
            <div className="wrap-sidebar-account">
              <div className="sidebar-account">
                <div className="account-avatar">
                  <div className="image"><img src="/template/storefront/images/avatar/user-account.jpg" alt="account" /></div>
                  <h6 className="mb_4">{client?.companyName ?? "Sarjan Client"}</h6>
                  <div className="body-text-1 text-secondary">{client?.email ?? "Login required"}</div>
                </div>
                <ul className="my-account-nav">
                  {nav.map((item) => (
                    <li key={item.href}>
                      {item.href === active ? (
                        <span className="my-account-nav-item active"><i className={`icon ${item.icon}`} />{item.label}</span>
                      ) : (
                        <a href={item.href} className="my-account-nav-item"><i className={`icon ${item.icon}`} />{item.label}</a>
                      )}
                    </li>
                  ))}
                </ul>
...
export function AccountDashboardPage() {
  ...
  const updatePassword = (key: keyof typeof password, value: string) => setPassword((current) => ({ ...current, [key]: value }));
  ...
  const saveProfile = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch(`/api/clients/${encodeURIComponent(client.id)}`, {
      ...
      body: JSON.stringify({
        ...
        gst: form.gst,
        city: form.city,
        address: { ...(client.address ?? {}), phone: form.phone, gst: form.gst, city: form.city },
      }),
    });
          ...
          <form className="form-account-details" onSubmit={saveProfile}>
            <div className="account-info">
              ...
              <div className="cols mb_20">
                <fieldset><input type="text" placeholder="Company Name*" value={form.companyName} onChange={(e) => updateForm("companyName", e.target.value)} required /></fieldset>
                <fieldset><input type="email" placeholder="Username or email address*" value={form.email} readOnly /></fieldset>
              </div>
              <div className="cols mb_20">
                <fieldset><input type="text" placeholder="Phone*" value={form.phone} onChange={(e) => updateForm("phone", e.target.value)} /></fieldset>
                <fieldset><input type="text" placeholder="GST Number" value={form.gst} onChange={(e) => updateForm("gst", e.target.value)} /></fieldset>
              </div>
              <div className="tf-select mb_20">
                <select className="text-title" value={form.city} onChange={(e) => updateForm("city", e.target.value)}>
              ...
              <div className="d-flex gap-12 flex-wrap mb_32">
                ...
                <a href="/my-account-orders" className="tf-btn btn-white has-border radius-4"><span className="text">View Orders</span></a>
    ...
    <AccountFrame active="/my-account-orders" title="Your Orders">
      <div className="account-orders">
        <div className="wrap-account-order">
          <table>
            <thead><tr><th className="fw-6">Order</th><th className="fw-6">Date</th><th className="fw-6">Status</th><th className="fw-6">Total</th><th className="fw-6">Actions</th></tr></thead>
            <tbody>
              {loading ? <tr><td colSpan={5}>Loading orders...</td></tr> : orders.length ? orders.map((order) => (
                <tr className="tf-order-item" key={order.id}>
                  <td>{order.id}</td>
                  <td>{new Date(order.createdAt).toLocaleDateString("en-IN")}</td>
                  <td>{order.status}</td>
                  <td>{money(order.subtotal)}</td>
                  <td><a href={`/my-account-orders-details?orderId=${encodeURIComponent(order.id)}`} className="tf-btn btn-fill radius-4"><span className="text">View</span></a></td>
                </tr>
...
export function AccountAddressPage() {
  ...
  const save = async () => {
    ...
    const res = await fetch(`/api/clients/${encodeURIComponent(client.id)}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ address, city: address.city, gst: address.gst, phone: address.phone }),
    });
    const data = await res.json();
    if (res.ok) {
      setClient(data.client);
      ...
      <div className="account-address">
        {loading ? <p>Loading address...</p> : (
          <div className="text-center widget-inner-address">
            <button type="button" className="tf-btn btn-fill radius-4 mb_20 btn-address"><span className="text text-caption-1">Add / Edit address</span></button>
            <form className="show-form-address wd-form-address sarjan-address-form" onSubmit={(event) => { event.preventDefault(); void save(); }}>
              <div className="title">Add a new address</div>
              <div className="cols mb_20">
                <fieldset><input type="text" placeholder="Contact Name*" value={address.contactName ?? ""} onChange={(e) => update("contactName", e.target.value)} /></fieldset>
                <fieldset><input type="text" placeholder="Phone*" value={address.phone ?? ""} onChange={(e) => update("phone", e.target.value)} /></fieldset>
              </div>
              <div className="cols mb_20">
                <fieldset><input type="text" placeholder="GST Number" value={address.gst ?? ""} onChange={(e) => update("gst", e.target.value)} /></fieldset>
                <fieldset><input type="text" placeholder="Transport" value={address.transport ?? ""} onChange={(e) => update("transport", e.target.value)} /></fieldset>
              </div>
              <fieldset className="mb_20"><input type="text" placeholder="Address" value={address.line1 ?? ""} onChange={(e) => update("line1", e.target.value)} /></fieldset>
              <fieldset className="mb_20"><input type="text" placeholder="Address line 2" value={address.line2 ?? ""} onChange={(e) => update("line2", e.target.value)} /></fieldset>
              <div className="cols mb_20">
                <fieldset><input type="text" placeholder="City" value={address.city ?? ""} onChange={(e) => update("city", e.target.value)} /></fieldset>
                <fieldset><input type="text" placeholder="State" value={address.state ?? ""} onChange={(e) => update("state", e.target.value)} /></fieldset>
              </div>
              <fieldset className="mb_20"><input type="text" placeholder="Postal Code" value={address.pincode ?? ""} onChange={(e) => update("pincode", e.target.value)} /></fieldset>
              <div className="tf-cart-checkbox mb_20">
                <div className="tf-checkbox-wrapp"><input defaultChecked type="checkbox" id="address-default" /><div><i className="icon-check" /></div></div>
              ...
              <div className="d-flex align-items-center justify-content-center gap-20">
                <button type="submit" className="tf-btn btn-fill radius-4"><span className="text">Save address</span></button>
                <button type="button" className="tf-btn btn-white has-border radius-4" onClick={() => setAddress(client?.address ?? {})}><span className="text">Cancel</span></button>
              </div>
              {message ? <p className={message.includes("failed") || message.includes("required") ? "text-danger mt_16" : "text-success mt_16"}>{message}</p> : null}
            </form>
            <div className="list-account-address">
              <div className="account-address-item">
                <h6 className="mb_20">Default</h6>
                <p>{address.contactName || client?.companyName || "Sarjan Client"}</p>
                ...
                <div className="d-flex gap-10 justify-content-center">
                  <button type="button" className="tf-btn radius-4 btn-fill justify-content-center" onClick={() => document.querySelector<HTMLInputElement>(".sarjan-address-form input")?.focus()}><span className="text">Edit</span></button>
                  <button type="button" className="tf-btn radius-4 btn-white has-border justify-content-center" onClick={remove}><span className="text">Delete</span></button>
                </div>
              </div>
            </div>
          </div>
...
function OrderView({ order }: { order: Order }) {
  const steps = ["Pending approval", "Approved", "In Production", "Packed", "Ready for Dispatch", "Dispatched", "Delivered"];
      ...
      <div className="wd-form-order">
        <div className="order-head">
          <div className="content">
            ...
            <h6 className="mt_8">Thank you. Your B2B order request has been received.</h6>
          </div>
          <div className="text-end"><p className="text-secondary">Total</p><h5>{money(order.subtotal)}</h5></div>
        </div>
        <div className="account-order-item">
          {order.items.map((item) => (
            <div className="item-order" key={`${order.id}-${item.slug}-${item.color}`}>
              <div className="content">
                <h6>{item.name}</h6>
                <p className="text-secondary">{item.color} / {item.sizes.join(" / ")} / {item.setQuantity} set</p>
              </div>
        ...
        <div className="widget-tabs style-3 widget-order-tab mt_32">
          <ul className="widget-menu-tab"><li className="item-title active"><span className="inner">Tracking</span></li></ul>
...
export function AccountOrderDetailsPage({ orderId }: { orderId?: string }) {
  const { orders, loading } = useClientAndOrders();
  const order = useMemo(() => orderId ? orders.find((item) => item.id === orderId) : orders[0], [orderId, orders]);

  return (
    <AccountFrame active="/my-account-orders" title="Order Details">
      {loading ? <p>Loading order...</p> : order ? <OrderView order={order} /> : <p className="text-secondary">No order found.</p>}
    </AccountFrame>
  );
}
...
export function OrderTrackingPage() {
  ...
  const track = async () => {
    ...
    const data = await fetch(`/api/orders?${params.toString()}`).then((res) => res.json()).catch(() => ({ orders: [] }));
  ...
  return (
    <>
      <PageTitle title="Order Tracking" crumbs={["Homepage", "Order Tracking"]} />
      <section className="flat-spacing">
        <div className="container">
          <div className="login-wrap tracking-wrap">
            <div className="left">
              ...
              <form className="form-login" onSubmit={(event) => { event.preventDefault(); void track(); }}>
                <div className="wrap">
                  <fieldset>
                    <input className="sarjan-tracking-input" type="text" value={orderId} onChange={(e) => setOrderId(e.target.value)} placeholder="Order ID*" autoComplete="off" />
                  </fieldset>
                  <fieldset>
                    <input className="sarjan-tracking-input" type="email" value={billingEmail} onChange={(e) => setBillingEmail(e.target.value)} placeholder="Billing Email*" autoComplete="email" />
                  </fieldset>
                </div>
                <div className="button-submit">
                  <button className="tf-btn btn-fill" type="submit" disabled={tracking}><span className="text">{tracking ? "Tracking..." : "Tracking Orders"}</span></button>
                </div>
              </form>
            ...
            <div className="right">
              {client?.id ? (
                <>
                  ...
                  <p className="text-secondary">Open your order history for all saved B2B orders, approvals, and dispatch status.</p>
                  <a href="/my-account-orders" className="tf-btn btn-fill"><span className="text">My Orders</span></a>
                </>
              ) : (
                <>
                  <h4 className="mb_8">Already have an account?</h4>
                  <p className="text-secondary">Sign in to access order history, saved address, and B2B credit workflow.</p>
                  <a href="/login" className="tf-btn btn-fill"><span className="text">Login</span></a>
                </>
              )}
            </div>
          </div>
          {order ? <div className="mt_32"><OrderView order={order} /></div> : null}
        </div>
      </section>
    </>
  );
