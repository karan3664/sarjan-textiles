...
function WishlistPagination({ page, totalPages }: { page: number; totalPages: number }) {
      ...
      {Array.from({ length: totalPages }).map((_, index) => {
        ...
        return (
          <li className={nextPage === page ? "active" : ""} key={nextPage}>
            <a href={`/wishlist?page=${nextPage}`} className="pagination-item text-button">{nextPage}</a>
          </li>
        );
      })}
    </ul>
...
export function WishlistPageClient({ page = 1 }: { page?: number }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/catalog/products?ids=${encodeURIComponent(slugs.join(","))}&limit=${slugs.length}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const currentPage = Number.isFinite(page) ? Math.min(Math.max(Math.floor(page), 1), totalPages) : 1;
  const visibleProducts = useMemo(() => products.slice((currentPage - 1) * perPage, currentPage * perPage), [products, currentPage]);
  ...
  if (!products.length) {
    return (
      <div className="text-center py-5">
        ...
        <a href="/products" className="tf-btn btn-fill radius-4 mt_16"><span className="text">Browse Products</span></a>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb_32">
        ...
        <button type="button" className="tf-btn btn-white has-border radius-4" onClick={clearWishlist}><span className="text">Clear wishlist</span></button>
      </div>
      <div className="tf-grid-layout tf-col-2 lg-col-4">
        {visibleProducts.map((product, index) => <ModaveProductCard product={product} delay={`${index / 10}s`} key={product.id} />)}
      </div>
