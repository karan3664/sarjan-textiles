...
export function StorefrontModals() {
  ...
  return (
    <>
      <div className="modal fade modal-search" id="searchModal" tabIndex={-1} aria-hidden="true">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content p-4">
            ...
            <input className="sarjan-form-control mb-4" placeholder="Search products, fabric, SKU" />
            <div className="row g-3">
              {mockApi.products.slice(0, 3).map((product) => (
                <div key={product.id} className="col-4">
                  <Link href={`/products/${product.slug}`} className="text-decoration-none text-dark">
                    <img src={product.images[0]} alt={product.name} className="w-100 rounded mb-2" />
      ...
      <div className="modal fullRight fade" id="cartModal" tabIndex={-1} aria-hidden="true">
          ...
          <div className="modal-content h-100 rounded-0">
            ...
            <div className="modal-body">
              <div className="d-grid gap-3">
                {cartItems.map((item) =>
                  item ? (
                    <div key={`${item.productSlug}-${item.size}`} className="d-flex gap-3 border-bottom pb-3">
                      <img src={item.product.images[0]} alt={item.product.name} width={92} height={116} className="object-fit-cover rounded" />
