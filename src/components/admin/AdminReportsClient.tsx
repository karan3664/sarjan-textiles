...
type ReportKey = "orders" | "clients" | "inventory" | "finance" | "dispatch" | "productMovement";
...
async function downloadXlsx(filename: string, rows: Array<Record<string, any>>) {
...
function printPdf(title: string, rows: Array<Record<string, any>>) {
  const headers = Object.keys(rows[0] ?? { empty: "" });
...
export function AdminReportsClient({ data }: { data: AdminReportsData }) {
  ...
  const rows = useMemo(() => {
    const reportRows = data[active] as Array<Record<string, any>>;
    const normalized = query.trim().toLowerCase();
    if (!normalized) return reportRows;
    return reportRows.filter((row) => Object.values(row).some((value) => String(value ?? "").toLowerCase().includes(normalized)));
  }, [active, data, query]);
      ...
      <div className="sarjan-home-kpi-grid sarjan-products-kpi-grid">
        {data.summary.map((metric) => (
          <div className="sarjan-home-kpi-card" key={metric.label}>
            <div className="sarjan-home-kpi-icon"><i className="icon-chart-bar" /></div>
            <div><div className="body-text text-secondary">{metric.label}</div><h5>{metric.value}</h5></div>
          </div>
        ))}
      </div>
      <div className="wg-box sarjan-report-box">
        <div className="flex flex-wrap justify-between gap14 items-center mb-24">
          <div>
            ...
            <div className="body-text text-secondary">Orders, clients, inventory, dispatch, product movement, finance ledger, CSV, Excel, and PDF exports.</div>
          </div>
          <div className="d-flex gap10 flex-wrap">
            <button type="button" className="tf-button" onClick={() => downloadCsv(`${active}-report.csv`, rows)}>CSV</button>
            <button type="button" className="tf-button" onClick={() => downloadXlsx(`${active}-report.xlsx`, rows)}>Excel</button>
            <button type="button" className="tf-button" onClick={() => printPdf(reportLabels[active], rows)}>PDF</button>
          </div>
        </div>
        <div className="sarjan-report-tabs">
          {(Object.keys(reportLabels) as ReportKey[]).map((key) => (
            <button type="button" className={active === key ? "active" : ""} key={key} onClick={() => setActive(key)}>{reportLabels[key]}</button>
          ))}
        </div>
        <form className="form-search-2 sarjan-report-search" onSubmit={(event) => event.preventDefault()}>
          <fieldset className="name"><input className="show-search" placeholder="Search report" value={query} onChange={(event) => setQuery(event.target.value)} /></fieldset>
        ...
        <div className="wg-table table-product-list sarjan-report-table">
          <table>
            <thead><tr>{headers.map((header) => <th className="text-title" key={header}>{header}</th>)}</tr></thead>
            <tbody>
              ...
              {!rows.length ? <tr><td><div className="sarjan-empty-state">No report data found.</div></td></tr> : null}
            </tbody>
          </table>
        </div>
      </div>
