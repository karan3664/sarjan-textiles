...
import { LayoutDashboard, Package, ShoppingCart, UsersRound, FileText, Settings } from "lucide-react";
      ...
      <aside className="sarjan-admin-sidebar">
        ...
        <nav className="d-grid gap-2">
          {links.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className="d-flex align-items-center gap-2 text-dark text-decoration-none fw-bold p-3 rounded border">
                <Icon size={18} />
                {item.label}
              </Link>
            );
