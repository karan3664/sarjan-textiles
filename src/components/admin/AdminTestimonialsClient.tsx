...
import { TestimonialStarsDisplay } from "@/components/storefront/TestimonialStarRating";
...
export function AdminTestimonialsClient({
  ...
  const filteredTestimonials = useMemo(
    () =>
      testimonials.filter(
        (testimonial) => filter === "all" || testimonial.status === filter,
      ),
    [filter, testimonials],
  );
    ...
    <>
      <div className="sarjan-home-kpi-grid sarjan-testimonial-kpi-grid">
        {filters.map((item) => (
          <button
            type="button"
            className={`sarjan-home-kpi-card sarjan-filter-card${filter === item.value ? " active" : ""}`}
        ...
        <div className="flex flex-wrap justify-between gap14 items-center mb-24">
          <div>
            ...
            <div className="body-text text-secondary">
              Customer/frontend submissions appear here first. Only approved
              testimonials show on homepage.
            </div>
          </div>
          <div className="box-status text-button type-delivery">
            {filteredTestimonials.length} Showing
          </div>
        </div>
            ...
            <div className="sarjan-admin-testimonial-card" key={testimonial.id}>
              ...
              <div className="sarjan-admin-testimonial-content">
                <div className="sarjan-admin-testimonial-head">
                  ...
                  <div
                    className={`box-status text-button ${statusClass(testimonial.status)}`}
                  >
                    {testimonial.status}
                  </div>
                </div>
