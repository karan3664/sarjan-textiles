...
function productStatus(product: Product) {
  if (product.stock <= 0) return { label: "Out of Stock", className: "type-inactive", filter: "out" as const };
  if (product.stock - product.reserved <= product.moq) return { label: "Low Stock", className: "type-pending", filter: "low" as const };
  return { label: "Publish", className: "type-completed", filter: "published" as const };
}

export function AdminProductListClient({ initialProducts }: { initialProducts: Product[] }) {
  ...
  const visibleProducts = useMemo(() => {
    ...
    const filtered = products.filter((product) => {
      const matchesQuery = !normalizedQuery || [product.name, product.sku, product.category, product.fabric].some((value) => value.toLowerCase().includes(normalizedQuery));
      const matchesCategory = category === "all" || product.category === category;
      const matchesStatus = status === "all" || productStatus(product).filter === status;
  ...
  const deleteProduct = async (product: Product) => {
    ...
    try {
      const res = await fetch(`/api/admin/cms/products?slug=${encodeURIComponent(product.slug)}`, { method: "DELETE" });
    ...
    <>
      <div className="sarjan-home-kpi-grid sarjan-products-kpi-grid">
        {[
          ...
          ["Published", products.filter((product) => productStatus(product).filter === "published").length, "icon-sealCheck"],
          ["Low Stock", products.filter((product) => productStatus(product).filter === "low").length, "icon-basket"],
          ["Out of Stock", products.filter((product) => productStatus(product).filter === "out").length, "icon-close"],
      ...
      <div className="flex flex-wrap justify-between gap14 items-center mb-24">
        <div>
          <div className="body-text text-secondary">Product catalog loaded from CMS/backend data.</div>
        </div>
        <Link href="/admin/products-create" className="tf-button text-btn-uppercase">
          Create New Product
        </Link>
      </div>
      ...
      <div className="wg-box sarjan-products-list-box">
        ...
        <div className="wg-table table-product-list list-item-function sarjan-products-table">
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              {paginatedProducts.map((product) => {
                ...
                return (
                  <tr className="tf-table-item item-row" key={product.id}>
                    ...
                    <td>
                      <div className="text-title name text-line-clamp-1">{product.category}</div>
                      <div className="text-caption-1 sub">{product.fabric}</div>
                    </td>
                    <td>{formatInr(product.price)}</td>
                    <td>
                      <div className={`box-status w-100 text-button ${statusInfo.className}`}>{statusInfo.label}</div>
                    </td>
                    <td className="text-end">
                      <div>{product.stock}</div>
                      <div className="text-caption-1 text-secondary">Reserved {product.reserved}</div>
                    </td>
                    <td className="d-flex gap8 justify-content-start">
                      <Link href={`/products/${product.slug}`} className="hover-tooltips tf-btn-small" target="_blank">
                        <i className="icon icon-eye" />
                        <span className="tooltips text-caption-1">Frontend Preview</span>
                      </Link>
                      <Link href={`/admin/products-create?slug=${encodeURIComponent(product.slug)}`} className="hover-tooltips tf-btn-small">
                        <i className="icon icon-edit" />
                        <span className="tooltips text-caption-1">Edit</span>
                      </Link>
                      <button type="button" className="hover-tooltips tf-btn-small btns-trash" disabled={deletingSlug === product.slug} onClick={() => deleteProduct(product)}>
              ...
              {!visibleProducts.length && (
                <tr>
                  <td colSpan={7}>
                    <div className="body-text text-secondary p-4">No products found.</div>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {visibleProducts.length > pageSize ? (
          <div className="sarjan-products-pagination">
            <div className="body-text text-secondary">
              Showing <span>{startIndex + 1}</span>-<span>{endIndex}</span> of <span>{visibleProducts.length}</span> products
            </div>
