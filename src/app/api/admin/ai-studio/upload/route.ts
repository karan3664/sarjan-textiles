...
export async function POST(request: Request) {
  ...
  try {
    const formData = await request.formData().catch((error) => {
      throw new Error(
        error instanceof Error && error.message.includes("multipart")
          ? "Upload body was truncated. Restart dev server after config change, then retry with images under 60MB each."
    ...
    if (oversized) {
      return Response.json(
        { error: `${oversized.name} exceeds 60MB` },
        { status: 400 },
      );
    }
    ...
    const result = await saveRawUploads(files, {
      category,
      collection,
      attributeType,
      attributeValue,
