...
export async function POST(request: Request) {
  try {
    ...
    const scope: ClientPricingRule["scope"] =
      body.scope === "category" ? "category" : "product";
    const categoryPath = Array.isArray(body.categoryPath)
      ? body.categoryPath
          .map((item: unknown) => String(item).trim())
          .filter(Boolean)
      : String(body.categoryPath ?? "")
          .split(",")
          .map((item) => item.trim())
          .filter(Boolean);
    ...
    if (
      ...
      (scope === "category" && !categoryPath.length)
    ) {
      return Response.json(
        { error: "Client and product/category required" },
        { status: 400 },
      );
    }
    const customPrice =
    ...
    const discountPercentage =
      body.discountPercentage === "" || body.discountPercentage === undefined
        ? undefined
        : Number(body.discountPercentage);

    const current = await getCmsSnapshot();
    const previous = body.id
      ? current.clientPricing.find((rule) => rule.id === body.id)
      : undefined;
    ...
    await appendAuditLog({
      actor: session.email,
      role: session.role,
      action: previous ? "update_pricing_rule" : "create_pricing_rule",
      entity: "client_pricing",
      entityId: rule.id,
      before: previous,
      after: rule,
      note: rule.note,
    ...
    return Response.json(
      { error: error instanceof Error ? error.message : "Pricing save failed" },
      { status: 400 },
    );
  }
}
...
export async function DELETE(request: Request) {
  ...
  const before = (await getCmsSnapshot()).clientPricing.find(
    (rule) => rule.id === id,
  );
