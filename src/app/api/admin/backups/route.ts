...
import { createAppBackup, deleteAppBackup, listAppBackups, readAppBackup, restoreAppBackup } from "@/lib/admin-backups";
...
async function superAdmin() {
  ...
  if (session.role !== "super_admin") return { error: "Super admin only", status: 403 as const };
  return { session };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const created = await createAppBackup({ name: body.name, createdBy: auth.session.email, source: "manual" });
    return Response.json({ backups: await listAppBackups(), createdId: created.id });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Backup action failed" }, { status: 400 });
  }
}
...
export async function DELETE(request: Request) {
  ...
  const url = new URL(request.url);
  const id = url.searchParams.get("id");
  if (!id) return Response.json({ error: "Backup id required" }, { status: 400 });
