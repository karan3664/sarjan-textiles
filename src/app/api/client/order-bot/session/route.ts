...
function sessionErrorResponse(error: unknown, label: string) {
  console.error(`${label} failed`, error);
  const message =
    error instanceof Error ? error.message : "Could not start chat session";
...
export async function POST(request: Request) {
  try {
    ...
    return sessionErrorResponse(error, "order-bot session POST");
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return sessionErrorResponse(error, "order-bot session PATCH");
  }
}
