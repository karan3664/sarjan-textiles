...
export function getMobileAppRelease(): MobileAppRelease {
  ...
  const latestVersion =
    process.env.MOBILE_APP_LATEST_VERSION?.trim() || "1.0.1";
  const versionCode = Number(process.env.MOBILE_APP_VERSION_CODE || "2");
  const forceUpdate =
    process.env.MOBILE_APP_FORCE_UPDATE === "1" ||
    process.env.MOBILE_APP_FORCE_UPDATE === "true";
  const releaseNotes =
    process.env.MOBILE_APP_RELEASE_NOTES?.trim() ||
    "Mobile login OTP, catalog updates, and checkout improvements.";
